...
% how do you know define different veracity 
...
% gateway offloading:
%  this is used a lot in banking. apllication level sprayer. 

% API gateway: 
% it does make sense, quite a common pattern in baking, they do need to that offloading. I spoke to researhc guys at IBM when they where processing video streaming. 

% as you walk in the corridor, they would check your video against when you get out, and check if you see if you spent and bought that. that's because a lot of people pick up receip from trash and cause a lot of loss. 

% external configuration store: 

% familiar concept 

% declarative configuration management. did some work in Westpac to do their pipelines to build around their Kubernetes. This is inspired by it. Metadata object, very useful and I pushed it into service that was not a container. 

% metadata tagging is really useful. 


% velocity: 

% concepts are familiar. log aggregator is time-sychroonization and how critical time-sychroonization is. when you get a lot of data, and when you try to figure out what happened, that becomes a challenge in its own. 

% In banking that's getting down to millions of a second. So keep an eye on time-sychroonization. 

% whatever system you built should be self-aware, it should break silently. and make sure you feed event into aggregator. 

% If something hangs up, cricuit breaker can detect  that. If you run a piece of code, your code should report that. It would be really nice if you could change the logging level dynamically. 

% Can I turn up my logging level from errors only, to information messages to more. If I Can do it dynamically, iot's really useful. 


% variety:

% that works fine, and basically means you can progerssively you can roll out slowly. 

% Many years ago, EDI standars (electrocni data interchange) would take files with no checking. Because that's the way it works. And that was how it worked. This concept of spraying different formats in different places can be useful. 

% This also helps with team who can't work with that format. This means teams that don't have the skil to work with that format can drop that format into a bucket, and leave it there when the team is ready. 

% API gateway helps a lot. I do service transitions. The process of the change is as much as the end state 


% when ship comes in, you have a list of containers on that ship. It could be big ones, small ones, Refigerator ones. the standard was important to understand which one is important. that EDI information is crucially imporant. it was stock in almost teletype and nobody could do that. And it was hardcoded. 

% Value: 

% I can see where you going. It's not something I've done before. Requirements bloat is what you're talking about where everybody talks about. Dont setup an optimized read pattern unless there's a good business reason for it. 

% Securitry and privacy: 

% looks like SOA service brokers. I've got a request of TYPE A, and this is where you go and get it and it can change. It's a more advanced version of service brokers. 

% Big data people hate encryption. but end to end encryption is important for GDPR, as the cases for recent hacks. 

% if somebody gets your data, it should not mean it's compromised. Data should still be encrypted. Getting access to the database shouldn't give them access to data. if there's no encryption, you have pushed yourseslf back to old times. look into hardware encrypters. 

% Project Jericho was the city that fell down, and walls came down. There are no perimetes and there's no borders with data connectivity and if your data is not safe, and you depend on perimeters, you need to make sure your data perimete is very good, and there is no good one. 

% Another thing about privacy, privacy off-loading, deleting is important. You have to make sure you get rid of data. You might wanna get rid of it so you're not accountable for it. 

% If you're data is no longer required, is there any justification for simply deleting it. 

% veracity: 

% one thing that I would caution you. It's healthy when it starts, it's unhealthy when it finishes. akcnolwedgement is important, you need to figure this out. 

% You start to get into areas of re-processing unecessarily. But you need to understand how it's done, and how to signal it's done. Because circuit breaker job is not completed, untill the end of the filter. 

% You have to understand each of your filters are self-aware, and potentially allows circuit breakers to send evens for good and bad completion. 


% another comments: 

% it looks comprehensive. a lot of patterns are familiar, enhanced and improved. some of the old problems remain new. 
